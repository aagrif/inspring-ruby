...
describe RelativeSchedule do
  ...
  describe "schedule_errors" do
    it 'does not allow type to be anything outside of minute,hour,day and week' do
      ...
      expect(subject.schedule_errors).to_not be_nil
    end
    ...
    it 'validates arguments when type is Hour' do
      ...
      subject.relative_schedule_number = -10
      expect(subject.schedule_errors).to_not eq [nil,nil]
  ...
  describe "target_time" do
    let(:from_time) {Time.zone.local(2014,1,1,10,10)} #Wednesday
    it "returns right target time when type is minute" do
      subject.schedule = 'Minute 20'
      expect(subject.target_time(from_time)).to eq Time.zone.local(2014,1,1,10,30)
      subject.schedule = 'Minute 55'
      expect(subject.target_time(from_time)).to eq Time.zone.local(2014,1,1,11,5)
    end
    it "returns right target time when type is hour" do
      subject.schedule = 'Hour 2 15'
      expect(subject.target_time(from_time)).to eq Time.zone.local(2014,1,1,11,15)
      subject.schedule = 'Hour 1 15'
      expect(subject.target_time(from_time)).to eq Time.zone.local(2014,1,1,10,15)
    end
    it "returns right target time when type is day" do
      subject.schedule = 'Day 2 18:15'
      expect(subject.target_time(from_time)).to eq Time.zone.local(2014,1,2,18,15)
    end         
    it "returns right target time when type is day and hour has not passed" do
      subject.schedule = 'Day 1 18:15'
      expect(subject.target_time(from_time)).to eq Time.zone.local(2014,1,1,18,15)
      subject.schedule = 'Day 2 18:15'
      expect(subject.target_time(from_time)).to eq Time.zone.local(2014,1,2,18,15)
    end             
    it "returns right target time when type is week" do
      subject.schedule = 'Week 2 Tuesday 18:15'
      expect(subject.target_time(from_time)).to eq Time.zone.local(2014,1,14,18,15)
    end
    ...
    it "returns right target time when type is 1 week and day has passed" do
      ...
      expect(subject.target_time(from_time)).to eq Time.zone.local(2014,1,7,18,15)
    end        
    it "returns right target time when type is week and we are scheduling on the same day and time is past" do
      ...
      expect(subject.target_time(from_time)).to eq Time.zone.local(2014,1,8,8,0)
      subject.schedule = "Week 2 Wednesday 8:00"
      expect(subject.target_time(from_time)).to eq Time.zone.local(2014,1,15,8,0)   
      subject.schedule = "Week 1 Wednesday 10:00"
      expect(subject.target_time(from_time)).to eq Time.zone.local(2014,1,8,10,0)
      subject.schedule = "Week 2 Wednesday 10:00"
      expect(subject.target_time(from_time)).to eq Time.zone.local(2014,1,15,10,0)            
    end
    it "returns right target time when type is week and we are scheduling on the same day and time is in future" do
      ...
      expect(subject.target_time(from_time)).to eq Time.zone.local(2014,1,1,16,0)
      subject.schedule = "Week 2 Wednesday 16:00"
      expect(subject.target_time(from_time)).to eq Time.zone.local(2014,1,8,16,0)
      subject.schedule = "Week 1 Wednesday 10:15"
      expect(subject.target_time(from_time)).to eq Time.zone.local(2014,1,1,10,15)
      subject.schedule = "Week 2 Wednesday 10:15"
      expect(subject.target_time(from_time)).to eq Time.zone.local(2014,1,8,10,15)
    end
  end
