...
describe Message do
  ...
  it "pending_send returns messages whose next_send_time is in the past" do
    ...
    expect(Message.pending_send).to match_array [message2]
  end
  ...
  describe "#" do
    ...
    describe "perform_post_send_ops" do
      ...
      it "creates a message in the system secondary channel if reminder message required" do
        ...
        expect(smc.messages.last.options[:subscriber_ids]).to match_array [subscriber,subscriber2].map(&:id)
  ...
  describe "##" do
    ...
    it "schedule is validated when present" do
      ...
      expect(message.errors[:relative_schedule_minute]).to_not be_nil
    end
