...
describe Channel do
  ...
  it "allows moderator_emails to be blank" do
    expect_any_instance_of(EmailsValidator).to_not receive(:validate_each){}
  ...
  it "subscribers are not duplicates in a channel" do
    user = create(:user)
  ...
  it "subscriber is removed from sibling channels when added to new channel" do
    user = create(:user)
    ...
    expect(channel1.subscribers.length == 1).to be_truthy
    channel2.subscribers << subscriber
    channel1.reload
    expect(channel1.subscribers.length == 0).to be_truthy
    expect(channel2.subscribers.length == 1).to be_truthy
  end
  ...
  it "pending send scope returns those channels whose next_send_time is in past and not older than 8 hours" do
    prev_time = 2.days.ago.beginning_of_day
    Timecop.freeze(prev_time)
    channel1 = create(:random_messages_channel,next_send_time: prev_time - 9.hours)
    channel2 = create(:random_messages_channel,next_send_time: prev_time - 3.hours)
    channel3 = create(:random_messages_channel,schedule:IceCube::Rule.weekly.day((prev_time+3.days).strftime("%A").downcase.to_sym))
    expect(Channel.pending_send).to match_array [channel2]
    Timecop.return
  end
  ...
  describe "soft delete" do
    it "is supported" do
      ...
      expect(Channel.unscoped.size).to eq 2
    end
  end

  describe "#" do
    let(:tparty_keyword) {'+12025551212'}
    ...
    describe "with schedule upon creation updates next_send_time" do
      ...
      before do
        Timecop.freeze(Time.zone.local(2013,10,19,5,0)) #Saturday
        @channel = create(:channel,schedule:IceCube::Rule.daily.hour_of_day(9))
      end

      it "properly" do
        expect(@channel.next_send_time).to eq Time.zone.local(2013,10,19,9,0)
        @channel.schedule = IceCube::Rule.daily.hour_of_day(4)
        @channel.save
        expect(@channel.next_send_time).to eq Time.zone.local(2013,10,20,4,0)
        @channel.schedule = IceCube::Rule.weekly.day(:monday).hour_of_day(4)
        @channel.save
        expect(@channel.next_send_time).to eq Time.zone.local(2013,10,21,4,0)
        @channel.schedule = IceCube::Rule.weekly.day(:saturday).hour_of_day(0)
        @channel.save
        expect(@channel.next_send_time).to eq Time.zone.local(2013,10,26,0,0)
      end
      after do
        Timecop.return
      end
    ...
    # it doesn't know if its a subscriber response
    it "lists subscriber responses" do
      phone_number = Faker::PhoneNumber.us_phone_number
      sub = create(:subscriber, user:user, phone_number:phone_number)
      channel.subscribers << sub
      sub_responses_count = channel.subscriber_responses.count
      incoming_message = build :inbound_twilio_message
      incoming_message['From'] = Subscriber.format_phone_number(phone_number)
      incoming_message['To'] = tparty_keyword
      incoming_message['Body'] = Faker::Lorem.sentence
      imm = IncomingMessageManager.new(incoming_message)
      imm.process
      channel.reload
      expect(channel.subscriber_responses.count == sub_responses_count + 1).to be_falsey
    end
    ...
    describe "reset_next_send_time" do
      ...
      it "works fine when schedule is daily" do
        expect(@channel.next_send_time).to eq Time.zone.local(2013,10,19,9,0)
        Timecop.return
        Timecop.freeze(Time.zone.local(2013,10,26,5,0))
        @channel.reset_next_send_time
        expect(@channel.next_send_time).to eq Time.zone.local(2013,10,26,9,0)
        Timecop.return
      end
      it "works fine when schedule is weekly" do
        @channel.schedule = IceCube::Rule.weekly.day(:wednesday).hour_of_day(9)
        @channel.save
        expect(@channel.next_send_time).to eq Time.zone.local(2013,10,23,9,0)
        Timecop.return
        Timecop.freeze(Time.zone.local(2013,10,26,5,0))
        @channel.reset_next_send_time
        expect(@channel.next_send_time).to eq Time.zone.local(2013,10,30,9,0)
        Timecop.return
      end
    end
    ...
    describe "sent_messages_ids and #pending_messages_ids" do
      it "returns those messages which have been sent to this subscriber" do
        ...
        expect(channel.pending_messages_ids(subscriber)).to match_array (0..3).map{|i| messages[i].id}
        ...
        expect(channel.sent_messages_ids(subscriber)).to match_array (0..3).map{|i| messages[i].id}
    ...
    describe "send_scheduled_messages" do
      ...
      it "for internal messages, calls send_to_subscribers method of the message itself instead of MessagingManager" do
        ...
        expect_any_instance_of(ActionMessage).to receive(:send_to_subscribers){|message,subscribers|
          expect(subscribers).to match_array [subs1,subs2]
        }
        mm = double.as_null_object
        allow(MessagingManager).to receive(:init_instance){mm}
        expect(mm).to_not receive(:broadcast_message)
        subject.send_scheduled_messages
      end
      ...
      it "calls perform_post_send_ops for all messages" do
        ...
        allow(Message).to receive(:find_by){message_stub}
    ...
    describe "remove_keyword" do
      it "does not call the MessagingManagerWorker if there are other channels sharing the keyword" do
        ...
        expect(MessagingManagerWorker).to_not receive(:perform_async)
        ch1.destroy
      end
    end
    ...
    describe "process_custom_command" do
      ...
      xit "if not channel command, asks message to process subscriber response" do
        ...
        sr = create(:subscriber_response, message: message)
        allow(subject).to receive(:process_custom_channel_command){false}
        expect_any_instance_of(ResponseMessage).to receive(:process_subscriber_response)
      end
    end
