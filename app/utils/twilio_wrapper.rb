...
class TwilioWrapper
  ...
  def initialize(pclient = nil)
    @client = pclient || Twilio::REST::Client.new(ENV["TWILIO_SID"], ENV["TWILIO_AUTH_TOKEN"])
    @mock = if Rails.env.production? || Rails.env.production_local?
      false
    elsif @@mock_calls
      true
    else
  ...
  def send_message(phone_number, title, caption, content_url, from_num)
    if mock
      log "TWILIOMOCK send_message("\
          "#{phone_number}, #{title}, #{caption}, #{content_url}, #{from_num})"
      return true
    end

    Rails.logger.info "Called:TwilioWrapper#send_message("\
                      "#{phone_number}, #{title}, #{caption}, #{content_url}, #{from_num})"
