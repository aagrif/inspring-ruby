...
class OrderedMessagesChannel < Channel
  ...
  def group_subscribers_by_message
    ...
    h.each do |seq_no, subs|
      message = messages.find_by(seq_no: seq_no)
      msh[message.id] = subs if message
    end
  ...
  def perform_post_send_ops(msg_no_subs_hash)
    msg_no_subs_hash.each do |msg_id, subs|
      message = Message.find(msg_id)
      if message
        subs.each { |sub| sub.update_columns last_msg_seq_no: message.seq_no }
      end
    end
  end
end
