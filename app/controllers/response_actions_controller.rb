class ResponseActionsController < ApplicationController
  ...
  before_action :load_to_channel_options, only: %i(new edit create update)
  ...
  def create
    @response_action = ResponseActionFactoryService
    ...
    respond_to do |format|
      if @response_action.save
        format.html do
          redirect_to channel_message_response_actions_path(@channel, @message),
                      notice: "Response Action was successfully created."
        end
        format.json do
          render json: @response_action,
                 status: :created,
                 location: [@channel, @message, @response_action]
        end
      else
  ...
  def update
    @response_action = @message.response_actions.find(params[:id])
    @response_action = ResponseActionFactoryService
      .new(response_action_params, params, response_action: @response_action, message: nil)
      .response_action

    respond_to do |format|
      if @response_action.save
        format.html do
          redirect_to [@channel, @message, @response_action],
                      notice: "Response Action was successfully updated."
        end
  ...
  def import
    ...
    redirect_to session.delete(:import_requester) || request.referer,
                notice: "Response Actions imported."
  end
